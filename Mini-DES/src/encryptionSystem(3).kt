...
/**
 * Main function of problem 1 task 3. This program will take the plain text and use the above functions to perform the
 * desFunction on one 16-bit element. Printed in binary because 5 is not a multiple of 16.
...
fun main () {
    ...
    val key = "100000001111111010101100" //100000001111 111010101100

    val L0 = partitionedPlainTextInBinary[0].toString().slice(0..7)
    val R0 = partitionedPlainTextInBinary[0].toString().slice(8..15)

    val L1 = R0
    val R1 = xorStrings(L0, desFunction(R0, key.slice(0..11)))

    val L2 = R1
    val R2 = xorStrings(L1, desFunction(R1, key.slice(12..23)))

    println("The cipher text representation of plain text binary string ${partitionedPlainTextInBinary[0]} is: $L2$R2")
}
